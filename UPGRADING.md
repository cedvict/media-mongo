...
- You will also need to change the value of this configuration key as the previous class was removed, the new default value is `Cedvict\MediaLibrary\Support\FileNamer\DefaultFileNamer::class`
...
- the `Cedvict\MediaLibrary\HasMedia\HasMediaTrait` has been renamed to `Cedvict\MediaLibrary\InteractsWithMedia`. Make sure to update this in all models that use media.
...
- Url generation has been vastly simplified. You should set the `url_generator` in the `media-library` config file to `Cedvict\MediaLibrary\Support\UrlGenerator\DefaultUrlGenerator::class`. It will be able to handle most disks.
...
- if the `path_generator` key in the `media-library` config file was set to `null`, change the value to `Cedvict\MediaLibrary\Support\PathGenerator\DefaultPathGenerator::class`
- if the `url_generator` key in the `media-library` config file was set to `null`, change the value to `Cedvict\MediaLibrary\Support\UrlGenerator\DefaultUrlGenerator::class`
...
- rename the `use Cedvict\MediaLibrary\HasMedia\Interfaces\HasMedia;` interface to `use Cedvict\MediaLibrary\HasMedia\HasMedia;`
- rename the `use Cedvict\MediaLibrary\HasMedia\Interfaces\HasMediaConversions;` interface to `use Cedvict\MediaLibrary\HasMedia\HasMedia;` as well (the distinction was [removed](https://github.com/spatie/laravel-medialibrary/commit/48f371a7b10cc82bbee5b781ab8784acc5ad0fc3#diff-f12df6f7f30b5ee54d9ccc6e56e8f93e)).
...
- `Cedvict\MediaLibrary\Media` has been moved to `Cedvict\MediaLibrary\Models\Media`. Update the namespace import of `Media` accross your app
- The method definitions of `Cedvict\MediaLibrary\Filesystem\Filesystem::add` and `Cedvict\MediaLibrary\Filesystem\Filesystem::copyToMediaLibrary` are changed, they now use nullable string typehints for `$targetFileName` and `$type`.
