...
use Cedvict\MediaLibrary\Support\FileNamer\FileNamer;

/** @mixin \Cedvict\Image\Manipulations */
class Conversion
{
    ...
    public function __construct(string $name)
    {
        ...
        $this->manipulations = (new Manipulations())
            ->optimize(config('media-libraryc.image_optimizers'))
            ->format(Manipulations::FORMAT_JPG);

        $this->fileNamer = app(config('media-libraryc.file_namer'));

        $this->loadingAttributeValue = config('media-libraryc.default_loading_attribute_value');

        $this->performOnQueue = config('media-libraryc.queue_conversions_by_default', true);
    }
