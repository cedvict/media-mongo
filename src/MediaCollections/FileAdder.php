...
use Cedvict\MediaLibrary\MediaCollections\Exceptions\FileDoesNotExist;
use Cedvict\MediaLibrary\MediaCollections\Exceptions\FileIsTooBig;
use Cedvict\MediaLibrary\MediaCollections\Exceptions\FileUnacceptableForCollection;
use Cedvict\MediaLibrary\MediaCollections\Exceptions\UnknownType;
use Cedvict\MediaLibrary\MediaCollections\File as PendingFile;
use Cedvict\MediaLibrary\MediaCollections\Models\Media;
use Cedvict\MediaLibrary\ResponsiveImages\Jobs\GenerateResponsiveImagesJob;
...
class FileAdder
{
    ...
    public function toMediaCollectionFromRemote(string $collectionName = 'default', string $diskName = ''): Media
    {
        ...
        if ($storage->size($this->pathToFile) > config('media-libraryc.max_file_size')) {
            throw FileIsTooBig::create($this->pathToFile, $storage->size($this->pathToFile));
        }

        $mediaClass = config('media-libraryc.media_model');
        /** @var \Cedvict\MediaLibrary\MediaCollections\Models\Media $media */
    ...
    public function toMediaCollection(string $collectionName = 'default', string $diskName = ''): Media
    {
        ...
        if (filesize($this->pathToFile) > config('media-libraryc.max_file_size')) {
    ...
    protected function determineDiskName(string $diskName, string $collectionName): string
    {
        ...
        return config('media-libraryc.disk_name');
    }
    ...
    protected function processMediaItem(HasMedia $model, Media $media, self $fileAdder)
    {
        ...
        if ($this->generateResponsiveImages && (new ImageGenerator())->canConvert($media)) {
            $generateResponsiveImagesJobClass = config('media-libraryc.jobs.generate_responsive_images', GenerateResponsiveImagesJob::class);

            $job = new $generateResponsiveImagesJobClass($media);

            if ($customQueue = config('media-libraryc.queue_name')) {
